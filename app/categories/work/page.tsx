...
export default function WorkTasks() {
  ...
  const filteredTasks = tasks.filter((task) => task.category === "Work" && (task.title.toLowerCase().includes(searchQuery.toLowerCase()) || task.description.toLowerCase().includes(searchQuery.toLowerCase())));
    ...
    <div className="flex h-screen bg-[#14161B] text-white font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-275 mx-auto px-6 py-8 pt-20 md:p-10 lg:p-12">
          {/* Header & Stats & Tasks */}
          <DashboardHeader title="Work Tasks" subtitle={<>Stay focused and productive. You have {tasks.filter((t) => t.category === "Work" && !t.completed).length} pending tasks in Work.</>} tasks={tasks} searchQuery={searchQuery} setSearchQuery={setSearchQuery} onOpenModal={openModal}>
            {/* Work Tasks Section */}
            <div>
