...
export default function PersonalTasks() {
  ...
  const filteredTasks = tasks.filter((task) => task.category === "Personal" && (task.title.toLowerCase().includes(searchQuery.toLowerCase()) || task.description.toLowerCase().includes(searchQuery.toLowerCase())));
    ...
    <div className="flex h-screen bg-[#14161B] text-white font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-275 mx-auto px-6 py-8 pt-20 md:p-10 lg:p-12">
          {/* Header & Stats & Tasks */}
          <DashboardHeader title="Personal Tasks" subtitle={<>Stay on top of your personal life. You have {tasks.filter((t) => t.category === "Personal" && !t.completed).length} pending tasks in Personal.</>} tasks={tasks} searchQuery={searchQuery} setSearchQuery={setSearchQuery} onOpenModal={openModal}>
            {/* Personal Tasks Section */}
            <div>
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold">Personal Tasks</h3>
      ...
      <TaskModal isOpen={showModal} onClose={() => setShowModal(false)} onSave={handleSaveTask} editingTask={editingTaskData} defaultCategory="Personal" />
